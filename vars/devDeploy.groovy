	...
	node
	{
		...
		stage('Send Artifact to Nexus Repository'){
					 ...
					 {	
							...
							sh "mvn deploy:deploy-file -DgroupId=${groupId} -DartifactId=${artifactId} -Dversion=B${BUILD_NUMBER}-${version} -DgeneratePom=true -Dpackaging=jar -Durl=http://localhost:8091/repository/mule-releases/ -Dfile=${WORKSPACE}/target/${artifactId}-B${BUILD_NUMBER}-${version}-mule-application.jar"
